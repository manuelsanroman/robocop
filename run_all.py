...
class RobotController:
    ...
    """def ir_callback(self, msg):
        # Check if an object is detected within the range
        if msg.range <= 0.2:
            self.ir_range_queue.append(msg.range)  # Add the range reading to the queue
            self.consecutive_count += 1  # Increment the consecutive count
        else:
            self.ir_range_queue.clear()  # Clear the queue if a value greater than 0.3 is encountered
            self.consecutive_count = 0  # Reset the consecutive count

        # Set ir_detected to True if there are more than 10 consecutive values less than or equal to 0.3
        self.ir_detected = self.consecutive_count > 3
        self.consecutive_count = 0"""
